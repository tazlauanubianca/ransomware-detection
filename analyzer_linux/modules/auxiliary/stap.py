...
class STAP(Auxiliary):
    ...
    def start(self):
        ...
        log.warning("IT STARTED NOW NOW NOW")
        
        #while "systemtap_module_init() returned 0" not in self.proc.stderr.readline():
        #    pass

        line = self.proc.stderr.readline()
        while "systemtap_module_init() returned 0" not in line:
            log.debug("BIANCA: " + line)
            line = self.proc.stderr.readline()
        
        stap_stop = time.time()
        log.debug("STAP aux module startup took %.2f seconds" % (stap_stop - stap_start))
        return True
